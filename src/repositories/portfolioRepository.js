...
exports.createProjeto = async (projeto) => {
    ...
    INSERT INTO portfolio (titulo, link, image)
    VALUES ($1 , $2, $3)
    RETURNING *
    `, [projeto.titulo, projeto.link, projeto.image]);
    return result.rows[0];
};
...
exports.updateProjeto = async (id, projeto) => {
    const result = await pool.query(`
    UPDATE portfolio
    SET titulo = $1, link = $2, image = $3
    WHERE id = $4
    RETURNING * 
    `, [projeto.titulo, projeto.link, projeto.image, id]);
    return result.rows[0]; 
